...
This update keeps `messenger-edge-tool` aligned with the English-facing engineering portfolio while removing dash-line motion from SVG visuals.

## Changes

- Cleaned `assets/messenger-edge-motion.svg` so SVG motion no longer uses moving dashed or dotted line effects.
